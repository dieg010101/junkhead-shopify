(function () {
  ...
  const sizeChartCloseBtns = Array.from(
    root.querySelectorAll("[data-sizechart-close]")
  );
  ...
  function inventoryIsEnforced(variantId) {
    ...
    const tracked =
      ...
      String(v.inventoryManagement).toLowerCase() === "shopify";
  ...
  async function changeLine(line, quantity) {
    ...
    if (!res.ok) {
      try {
        ...
        throw new Error(
          data?.description || data?.message || "Unable to update cart."
        );
      } catch (_) {
    ...
    cartData.items.forEach((item, idx) => {
      ...
      row.innerHTML = `
        <div>
          ${
            item.image
              ? `<img class="jh__cartImg" src="${item.image}" alt="">`
              : ""
          }
        </div>
        <div>
          ...
          ${
            item.variant_title
              ? `<div class="jh__cartLineVariant">${item.variant_title}</div>`
              : ""
          }
          <div class="jh__cartLinePrice">${formatMoney(
            item.final_line_price
          )}</div>
        </div>
        <div style="display:grid; gap:8px; justify-items:end;">
          <input class="jh__qty" type="number" min="0" value="${
            item.quantity
          }" data-line="${line}">
  ...
  function getSizeMapFromVariants(product) {
    ...
    const map = variants
      .map((v) => {
        const label =
          v.options && v.options[sizeIdx] ? String(v.options[sizeIdx]) : "";
        return {
          label,
          variantId: v.id,
    ...
    const allNum =
      uniq.length && uniq.every((x) => !Number.isNaN(Number(x.label)));
  ...
  cartItemsEl?.addEventListener("change", async (e) => {
    ...
    try {
      ...
      const item =
        cart.items && cart.items[line - 1] ? cart.items[line - 1] : null;
